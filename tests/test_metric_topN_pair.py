...
def generate_and_eval(param):
    trace_name, top_n = param[0], param[1]
    ########## Preprocessor ##########
    ...
    ########## evaluation-iat ##########
    metric_dict = utils.metric_topN_pair(dfs=[data.trace_input[['srcip', 'dstip', 'iat', 'size']].copy(deep=True), \
    ...
    for key, val in metric_dict.items():
        df_tmp = pd.DataFrame({"model": [key]*len(val), "metric": val, 
                               "top": [f'top_{i+1}' for i in range(top_n)]})
        df_tmps.append(df_tmp)
    df_res = pd.concat(df_tmps)
    df_res['trace'] = trace_name
    return df_res
...
if __name__ == "__main__":
    ...
    for trace_name in trace_names:
        params.append([trace_name, top_n])
    ...
    df_eval.to_csv("df_eval_topPair_1.csv", index=False)
    pdb.set_trace()